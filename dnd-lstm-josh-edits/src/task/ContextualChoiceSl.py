...
class ContextualChoiceSl():
    ...
    def _sample_n_trials(self, n_examples):
        ...
        context = np.zeros((n_examples, self.trial_length, n_examples))
        target = np.zeros((n_examples, self.trial_length, n_examples))
        for i in range(n_examples):
            observation[i], context[i], target[i] = self._sample_one_trial(n_examples)
        return observation, target, context

    def _sample_one_trial(self, n_examples):
        """
        evidence:
            initially ambiguous,
        ...
        return evidence, cue, cue #target
